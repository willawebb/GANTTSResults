...
def stft(x, fft_size, hop_size, win_size, window):
    ...
    """
    x_stft = torch.stft(x, fft_size, hop_size, win_size, window.cuda())
    real = x_stft[..., 0]
    imag = x_stft[..., 1]
...
class STFTLoss(nn.Module):
    ...
    def forward(self, predicts, targets):
        ...
        """
        predicts_mag = stft(predicts, self.fft_size, self.hop_size, self.win_size, self.window.cuda())
        targets_mag = stft(targets, self.fft_size, self.hop_size, self.win_size, self.window.cuda())
